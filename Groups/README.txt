...
Server can take group requests from clients. Clients can request to add other clients to a group with #group protocol.
Server can remove clients from groups with the #ungroup protocal.
Server can broadcast statuses to groups with #gstatus protocal.
...
Client can add a friended client to a group with @add <group> <username>.
Client can remove another client from a group with @delete <group> <username>.
Client can send a status to a group with @send <group> <message>.
...
Max group size is hardcoded, could be added as another parameter.
Server doesn't check if a user is already in a group when someone adds them to it, so they may get duplicate notifications.
